...
public class allserv extends HttpServlet {
	...
	protected void save(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
		...
		try {
			...
			sn.executeUpdate("insert into tbl1(first_name,last_name,user_name,password) values('" + s2 + "','" + s3
					+ "','" + s4 + "','" + s5 + "')");
			sn.close();
			cn.close();
	...
	protected void search(HttpServletRequest request, HttpServletResponse response)
		...
		try {
			...
			while (r.next()) {
				...
				o.write("<td>" + fn + "</td>");
				o.write("<td>" + ln + "</td>");
				o.write("<td> <a href='allserv?flag=delete&id=" + i + "'>Delete</a> </td>");
				o.write("<td> <a href='allserv?flag=edit&id=" + i + "'>Edit</a> </td>");
	...
	protected void edit(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
		...
		try {
			...
			out.println("<form action='allserv' method='post'>");
	...
	protected void update(HttpServletRequest request, HttpServletResponse response)
		...
		try {
			...
			Connection cn = DriverManager.getConnection("jdbc:mysql://localhost/servjdbc", "root", "root");
			Statement sn = cn.createStatement();
			sn.executeUpdate("update tbl1 set first_name = '" + s2 + "' , last_name = '" + s3 + "' , user_name='" + s4
					+ "' , password = '" + s5 + "' where id =  " + id);
			sn.close();
			cn.close();
		} catch (ClassNotFoundException e) {
			// TODO Auto-generated catch block
	...
	protected void form(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
		...
		out.println("User Name<input type='text' name='un'><br>");
		out.println("Password <input type='password' name='password'><br><br>");
		out.println("<input type='hidden' name='flag' value='save'>");
		out.println("<input type='submit' value='Submit'></form><a href='allserv?flag=search'>Search</a>");
	}
}
